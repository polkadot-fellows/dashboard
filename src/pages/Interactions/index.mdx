...
### [Becoming a Polkadot Technical Fellowship Member](<!-- TODO: Link to updated overview page -->)

Please refer to the overview page.

### Salary Cycle Initialization, Induction & Continuance
The salary pallet has been [initiated](<!-- TODO: Link to on-chain event -->), members can now `induct` themselves into the payroll cycle(i.e. 30DAY periods)

<!-- TODO: Image with call to `induct` -->
...
<!-- TODO: Image with call to `submit_evidence` -->

Any account on the collectives system chain can submit a preimage of the call `approve` for ranked members of the fellowship and, submit in the appropriate track.
...
In the event of failed payments members can call `check_payment` to retry within the same cycle `status.cycle_index`
