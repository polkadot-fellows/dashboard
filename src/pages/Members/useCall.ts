...
export const transformIdentity = <T>(value: unknown): T => value as T

const isMapFn = (fn: unknown): fn is QueryMapFn =>
  !!(fn as QueryTrackFn).meta?.type?.isMap

const isQuery = (fn: unknown): fn is QueryableStorageEntry<"promise", []> =>
  !!fn && !isUndefined((fn as QueryableStorageEntry<"promise", []>).creator)

// extract the serialized and mapped params, all ready for use in our call
const extractParams = <T>(
  fn: unknown,
  params: unknown[],
...
export const unsubscribe = (tracker: TrackerRef): void => {
  ...
  nextTick((): void => {
    if (mountedRef.current) {
      ...
      if (canQuery) {
        ...
        tracker.current.subscriber = (fn as CallFn)(
          ...params,
          (value: Codec): void => {
...
export const throwOnError = (tracker: Tracker): void => {
  if (tracker.error) {
